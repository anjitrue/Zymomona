...
df <- ETDIGVTGGGQGK_FAIMS_light_summed
df_heavy <- ETDIGVTGGGQGK_FAIMS_heavy_summed
target_ions <- ETDIGVTGGGQGK_target_ions
...
FoldChange_DFI <- function(df, df_heavy, target_ions){
 ...
 df <- ETDIGVTGGGQGK_FAIMS_light_summed
 df_sum <- ETDIGVTGGGQGK_FAIMS_light_summed[,grep("Summed", colnames(ETDIGVTGGGQGK_FAIMS_light_summed))]
...
FC_DIF_within <- function(df,df_sum){
  ...
  WT <- df_sum[grep("WT",rownames(df_sum)),]
    ...
    for (j in 1:nrow(df_sum)){
        ...
        if(length(grep("120", rownames(df_sum)[i])) != 0){
...
AIEIVDQALDR_DIF_ratio$Sample <- paste0(AIEIVDQALDR_DIF_ratio$Sample, "_", AIEIVDQALDR_FAIMS_ratio$IonMobility)
...
CV_LC_Infusion <- rbind(aggregate_ETDIGVTGGGQGK,aggregate_AIEIVDQALDR, aggregate_sum_ETDIGVTGGGQGK, aggregate_sum_AIEIVDQALDR, aggregate_ratio_ETDIGVTGGGQGK, aggregate_ratio_AIEIVDQALDR)
...
Type[grep("ratio", CV_LC_Infusion$Sample)] <- paste0(Type[grep("ratio", CV_LC_Infusion$Sample)],"_","HeavyToLight")
...
meta <- FAIMS_Induction_ETDIGVTGGGQGKlight_meta
df <- FAIMS_Induction_ETDIGVTGGGQGKlight
...
df_heavy <- FAIMS_Induction_ETDIGVTGGGQGKheavy_ordered_selectiveIons
df_light <- FAIMS_Induction_ETDIGVTGGGQGKlight_ordered_selectiveIons
...
p <- ggplot(FAIMS_Induction_AIEIVDQALDR_ratio[-grep("-1",FAIMS_Induction_AIEIVDQALDR_ratio$UniqueWell),], aes(x=Timepoint, y=Ratio, color = as.factor(Strain))) +
  scale_y_continuous(breaks = seq(0,4,1), limit = c(0,4))+
  geom_jitter(stat = "identity", position = "jitter", size = 2.75, shape = 19, alpha = 0.6)+
  scale_color_manual(values = c("#5ABCB2", "#C3DE79","#A394DD"))+
  theme_light()
